...
public class Ex6 extends Applet
{
	...
	public static final byte INS_SET_KEY		= (byte) 0x02;
	public static final byte INS_PERFORM_DES	= (byte) 0x04;
	
	public static final byte INS_KEY_PAIR		= (byte) 0x06;
	public static final byte INS_GET_PUBKEY		= (byte) 0x08;
	public static final byte INS_PERFORM_RSA	= (byte) 0x0A;

	public static final byte INS_GET_RANDOM		= (byte) 0x0C;
...
//	private File currentFile;
	...
	private Cipher desCipher, rsaCipher;
	private RandomData rand;
	

	private final byte CURRENT_DIR 	= (byte) 0;
	private final byte CURRENT_FILE	= (byte) 1;
	
	private Object[] sessionObjs;
	
	protected Ex6()
	{
		masterFile = RootDirectory.getInstance();
		sessionObjs = JCSystem.makeTransientObjectArray((short) 2, JCSystem.CLEAR_ON_DESELECT);
	...
	public void process(APDU apdu)
	{
		...
		switch ( ins )
		{
			...
			case INS_SET_KEY : processSetKey(apdu);
				break;

			case INS_PERFORM_DES : processPerformDes(apdu);
				break;

			case INS_KEY_PAIR : processKeyPair(apdu);
				break;

			case INS_GET_PUBKEY : processGetPubKey(apdu);
				break;

			case INS_PERFORM_RSA : processPerformRsa(apdu);
				break;

			case INS_GET_RANDOM : processGetRandom(apdu);
				break;
	...
	private void setCurrentFile(File selectedFile)
	{
		...
		{
			sessionObjs[CURRENT_DIR] = selectedFile;
			sessionObjs[CURRENT_FILE] = null;
		}
		else
			sessionObjs[CURRENT_FILE] = selectedFile;
	}
	...
	private void processSelectFile(APDU apdu)
	{
		...
		Directory currentDir = (Directory) sessionObjs[CURRENT_DIR];
	...
	private void processDeleteFile(APDU apdu)
	{
		byte[] buffer = apdu.getBuffer();
		Directory currentDir = (Directory) sessionObjs[CURRENT_DIR];
		File currentFile = (File) sessionObjs[CURRENT_FILE];
...
//			currentDir.remove((byte)0);
	...
	private void processSetKey(APDU apdu)
	{
		byte[] buffer = apdu.getBuffer();
	...
	private void processPerformDes(APDU apdu)
	{
		byte[] buffer = apdu.getBuffer();
		byte mode = (byte) 0;
		...
		desCipher.init(desKey, mode);
		
		length = desCipher.doFinal(buffer, ISO7816.OFFSET_CDATA, length, buffer, (short) 0);
		apdu.setOutgoingAndSend((short) 0, length);
	}
	
	private void processKeyPair(APDU apdu)
	{
		byte[] buffer = apdu.getBuffer();
	...
	private void processGetPubKey(APDU apdu)
	{
		...
		length += pubKey.getModulus(buffer, (short) 0);
		length += pubKey.getExponent(buffer, length);
		apdu.setOutgoingAndSend((short) 0, length);
	}
	
	private void processPerformRsa(APDU apdu)
	{
		byte[] buffer = apdu.getBuffer();
		byte mode = (byte) 0;
		Key theKey = null;
		...
		switch ( buffer[ISO7816.OFFSET_P1] )
		{
			case 0 :
				mode = Cipher.MODE_ENCRYPT;
				theKey = (RSAPublicKey) rsaKeyPair.getPublic();
				break;
				
			case 1 :
				mode = Cipher.MODE_DECRYPT;
				theKey = (RSAPrivateCrtKey) rsaKeyPair.getPrivate();
				break;
		...
		rsaCipher.init(theKey, mode);
		
		length = rsaCipher.doFinal(buffer, ISO7816.OFFSET_CDATA, length, buffer, (short) 0);
		apdu.setOutgoingAndSend((short) 0, length);
	}
	
	private void processGetRandom(APDU apdu)
	{
		byte[] buffer = apdu.getBuffer();
		...
		rand.generateData(buffer, (short) 0, length);
		apdu.setOutgoingLength(length);
		apdu.sendBytes((short) 0, length);
	}
